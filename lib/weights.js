...
// Given how many questions are AVAILABLE in each category (not the actual
// question objects) and a target total, compute how many questions should be
// drawn from each category this session, proportional to weight — same
// redistribution logic as weightedSample, but returns counts only so the
// caller can pull actual questions via a shuffle-bag (see lib/shuffleBag.js)
// instead of a fresh random pick each time.
export function computeCategoryShares(availableCounts, totalWanted) {
  let remaining = Object.keys(availableCounts).filter((c) => availableCounts[c] > 0);
  let remainingWanted = totalWanted;
  const shares = {};
  for (const c of remaining) shares[c] = 0;

  while (remainingWanted > 0 && remaining.length > 0) {
    const totalWeight = remaining.reduce((sum, c) => sum + weightFor(c), 0);
    let pickedThisRound = 0;
    const stillShort = [];

    for (const cat of remaining) {
      const avail = availableCounts[cat] - shares[cat];
      const shareCalc = Math.max(
        1,
        Math.round((weightFor(cat) / totalWeight) * remainingWanted)
      );
      const take = Math.min(shareCalc, avail);
      shares[cat] += take;
      pickedThisRound += take;

      if (availableCounts[cat] - shares[cat] > 0) stillShort.push(cat);
    }

    remainingWanted =
      totalWanted - Object.values(shares).reduce((a, b) => a + b, 0);
